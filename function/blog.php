...
function tampil_kategori_admin() {
  $query = "SELECT *  FROM tags ORDER BY id_tag ASC";
  return result ($query);
}
...
function tambah_data ($judul, $konten, $i, $kategori) {
  $judul = escape($judul);
  $konten = escape($konten);
$query = "INSERT INTO artikel (judul, isi, gambar, id_kategori, hitung) VALUES ('$judul', '$konten', '$i', '$kategori', '0')";
return run ($query);
}
...
function artikel_id($judul, $konten, $kategori, $id) {
$query = "UPDATE artikel SET judul='$judul', isi='$konten', id_kategori='$kategori' WHERE id_artikel=$id";
return run ($query);
}

function edit($judul, $konten, $kategori, $i ,$id) {
$query = "UPDATE artikel SET judul='$judul', isi='$konten', id_kategori='$kategori' ,gambar='$i' WHERE id_artikel=$id";
return run ($query);
}
